...
- **`RaycastResolver`** — cross-platform raycast cascade returning the first valid hit: (1) native ARKit LiDAR raycast via `NativeLidar` (iPhone/iPad Pro; depth-aware, snaps to real geometry), (2) `ARRaycastManager` (planes / ARCore Depth / feature points), (3) a fallback point along the camera ray. Always go through this rather than calling AR raycasting directly.
- **Native LiDAR** (`NativeLidar` + `Assets/Plugins/iOS/LidarNative.mm`) — talks to the real ARKit `ARSession` directly (obtained from `XRSessionSubsystem.nativePtr`), **not** through AR Foundation's `ARMeshManager` (the old `LiDARScanner` mesh-occluder approach was removed). `NativeLidarDriver` registers the session and re-asserts `sceneDepth` + `sceneReconstruction` on the native config at 1 Hz (AR Foundation can clobber it when the image library restarts). Exposes depth-aware raycast and depth-map point capture. No-op in Editor/Android.
- **LiDAR mapping** (`LidarMapController` + `LidarPointCloud`, FSM mode `LidarMap`) — alternative flow: sweep the room and accumulate an anchor-relative point cloud (min-distance filter driven by a UI slider; spatial-hash dedupe). Points persist as a binary sibling `<name>.pts` next to the scan json (and inside `.mscn` packages, format v2). Rendered with `MeshTopology.Points` + `Resources/PointCloud.shader`. In Editor a fake generator produces points so the flow is testable.
- **Sorken spawns** (`SpawnBuilder` + `SorkenSpawnObject`) — spawn markers for the Sorken: a flattened pyramid whose tip points along the wall normal. Two flows: pick a `WallObject` (manual scan; takes its room-facing normal) or free placement using the LiDAR hit normal (mapping flow). Movable/rotatable with the transform gizmo; persisted in `ScanData.spawns`.
